...
import type { PdfTextPage } from "./ra";
...
export async function extractTextFromPdf(pdfBuffer: Buffer): Promise<string> {
  ...
  for (let p = 1; p <= pdf.numPages; p++) {
    ...
    const items = (content.items as PdfTextContentItem[]).filter((i) => i.str && i.str.trim() !== "");
    items.sort((a, b) => {
    ...
    for (const item of items) {
      ...
      const width = item.width || (itemText.length * 5); // Fallback width estimation if missing
...
export async function extractTextPagesFromPdf(pdfBuffer: Buffer): Promise<PdfTextPage[]> {
  ...
  for (let p = 1; p <= pdf.numPages; p++) {
    ...
    pages.push({
      ...
      items: (content.items as PdfTextContentItem[])
        .filter((item) => item.str && item.str.trim() !== "")
        .map((item) => ({
          str: item.str ?? "",
          x: item.transform[4],
          y: viewport.height - item.transform[5],
          width: item.width || (item.str?.length ?? 0) * 5,
          height: item.height,
        })),
    });
  }
