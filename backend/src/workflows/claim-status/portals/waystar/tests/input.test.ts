...
test("reads Waystar claim workbook rows and sends invalid rows to the error bucket", async () => {
  ...
  const sheet = XLSX.utils.json_to_sheet([
    { "Patient Name": "John Doe", Group: "Alpha Medical", "Claim Number": "ABC123", "Responsible Payer": "Medicare", DOS: "01/15/2026" },
  ...
  assert.equal(parsed.claimRows[0].group, "Alpha Medical");
