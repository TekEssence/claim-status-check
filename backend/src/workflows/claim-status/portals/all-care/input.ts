...
import type { AllCareCredentialBatch, AllCareCredentials, AllCareInput, AllCareInputRow } from "./types";
...
function readRows(buffer: ArrayBuffer): Record<string, unknown>[] {
  ...
  if (!sheet) throw new Error("AllCare workbook does not contain a worksheet.");
  return XLSX.utils.sheet_to_json(sheet, { raw: false, defval: "" }) as Record<string, unknown>[];
}

export function readAllCareCredentials(buffer: ArrayBuffer): AllCareCredentials[] {
  return readRows(buffer).map((row, index) => {
    const group = findValue(row, ["Group", "IPA", "Provider Group"]);
    const payer = findValue(row, ["Responsible Payer", "Responsible Party", "Payer", "Payer Name"]);
    ...
    if (missing.length) throw new Error(`AllCare login Excel row ${index + 2} is missing ${missing.join(", ")}.`);
    return { group, payer, loginUrl: normalizeUrl(rawUrl), username, password };
  });
}

export function readAllCareInputRows(buffer: ArrayBuffer): AllCareInputRow[] {
  return readRows(buffer).map((row, index) => {
    const group = findValue(row, ["Group", "IPA", "Provider Group"]);
    const payer = findValue(row, ["Responsible Payer", "Responsible Party", "Payer", "Payer Name"]);
    const memberId = findValue(row, ["Member ID", "Member Id", "ID", "Subscriber ID", "Subscriber No"]);
    ...
    } satisfies AllCareInputRow;
  }).filter((row) => row.group || row.payer || row.memberId || row.memberName);
}

export function routeAllCareRows(rows: AllCareInputRow[], credentials: AllCareCredentials[]): {
  batches: AllCareCredentialBatch[];
...
export async function parseAllCareInput(formData: FormData): Promise<AllCareInput> {
  const credentialExcel = formData.get("credentialExcel");
  const inputExcel = formData.get("inputExcel");
  if (!(credentialExcel instanceof File)) throw new Error("Missing AllCare login Excel file.");
  if (!(inputExcel instanceof File)) throw new Error("Missing AllCare claim Excel file.");
  return {
