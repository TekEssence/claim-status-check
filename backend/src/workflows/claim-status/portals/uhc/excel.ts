...
const MONEY_BOT_FIELDS = new Set<string>(['BotPaidAmount', 'BotBilledAmount']);

function parseMoneyCellValue(value: unknown): number | null {
  if (typeof value === 'number' && Number.isFinite(value)) return value;
  const text = String(value ?? '').trim();
  if (!text) return null;
  const isAccountingNegative = text.includes('(') && text.includes(')');
  const isLeadingNegative = text.startsWith('-');
...
function findCol(headerRow: ExcelJS.Row, ...names: string[]): number {
  ...
  headerRow.eachCell((cell, colNum) => {
    ...
    for (const name of names) {
      ...
      if (cellText === target || cellText.includes(target)) {
        found = colNum;
        break;
      }
    }
...
function findExactCol(headerRow: ExcelJS.Row, ...names: string[]): number {
  let found = -1;
  headerRow.eachCell((cell, colNum) => {
    const cellText = getCellText(cell).toLowerCase().trim();
    for (const name of names) {
      if (cellText === name.toLowerCase().trim()) {
        found = colNum;
        break;
      }
    }
...
export async function writeBotFields(
  ...
  for (const [key, value] of Object.entries(fields)) {
    ...
    if (colNum) {
      ...
      if (MONEY_BOT_FIELDS.has(key)) {
        const moneyValue = parseMoneyCellValue(value);
        if (moneyValue !== null) {
