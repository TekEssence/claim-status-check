...
const SUPPORTED_PAYER_PATTERN = /\b(aetna|blue\s*cross|blue\s*shield|bcbs|bcbstx|wellpoint|wellcare)\b/i;
...
function assertSupportedPayers(rows: AvailityInputRow[]): void {
  ...
  if (unsupported.length) {
    ...
    throw new Error(`Availity supports only Aetna, Blue Cross Blue Shield, Wellpoint, and Wellcare. Unsupported payer(s): ${unique.join(", ")}`);
  }
}
