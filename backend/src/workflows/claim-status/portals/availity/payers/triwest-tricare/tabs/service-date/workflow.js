...
async function processTriwestTricareServiceDateResults(page, row, provider, resultSummary, options = {}) {
  ...
  if (matchedRows.length === 0) {
    ...
    const mismatchReason = `Portal returned ${returnedCount} rows in ${sourceTab} for provider ${provider}, but none matched input ${matchCriteria}. ${mismatchDetail} ${returnedRowsSummary}`.trim();
    return {
      status: "failed",
...
async function searchTriwestTricareServiceDatesWithProvider(page, providerName, rowData) {
...
async function processClaim(page, row, options = {}) {
  ...
  for (const provider of providerOrder) {
    await searchTriwestTricareServiceDatesWithProvider(page, provider, row.data);
