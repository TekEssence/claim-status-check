...
async function processHealthNetServiceDateResults(page, row, provider, resultSummary, options = {}) {
  ...
  if (matchedRows.length === 0 && options.projectId === "medrevenu" && inputMemberId && inputPatientName) {
    logger.info("No Health Net Service Dates rows matched Medrevenu Member ID. Falling back to Patient Name match.");
  ...
  if (matchedRows.length === 0 && options.projectId === "medrevenu" && inputPatientNameWithoutInitial) {
    logger.info("No Health Net Service Dates rows matched exact Medrevenu Patient Name. Falling back to Patient Name without trailing initial.");
    matchedRows = resultRows.filter((result) => {
      return result.serviceDate === inputDate
        && normalizeMoney(result.billedAmount) === inputCharge
...
async function searchHealthNetServiceDatesWithProvider(page, providerName, rowData) {
...
async function processClaim(page, row, options = {}) {
  ...
  for (const provider of providerOrder) {
    await searchHealthNetServiceDatesWithProvider(page, provider, row.data);
