...
async function processTriwestVaCcnServiceDateResults(page, row, provider, resultSummary, options = {}) {
  ...
  if (matchedRows.length === 0 && options.projectId === "medrevenu" && inputMemberId && inputPatientName) {
    logger.info("No TRIWEST-VA CCN Service Dates rows matched Medrevenu Member ID. Falling back to Patient Name match.");
  ...
  if (matchedRows.length === 0 && options.projectId === "medrevenu" && inputPatientNameWithoutInitial) {
    logger.info("No TRIWEST-VA CCN Service Dates rows matched exact Medrevenu Patient Name. Falling back to Patient Name without trailing initial.");
    matchedRows = resultRows.filter((result) => {
      return result.serviceDate === inputDate
        && (!shouldMatchBilledAmount || normalizeMoney(result.billedAmount) === inputCharge)
...
async function searchTriwestVaCcnServiceDatesWithProvider(page, providerName, rowData) {
...
async function processClaim(page, row, options = {}) {
  ...
  for (const provider of providerOrder) {
    await searchTriwestVaCcnServiceDatesWithProvider(page, provider, row.data);
