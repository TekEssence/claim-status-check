...
const IEHP_CLAIM_FILE_KEY = "iehp-claim-file";
...
export async function saveIehpClaimFile(file: File): Promise<void> {
  await setValue(IEHP_CLAIM_FILE_KEY, file);
}

export async function loadIehpClaimFile(): Promise<File | null> {
  return getValue<File>(IEHP_CLAIM_FILE_KEY);
}

export async function clearIehpClaimFile(): Promise<void> {
  await deleteValue(IEHP_CLAIM_FILE_KEY);
}
...
export async function clearStoredRunContext(): Promise<void> {
  await Promise.allSettled([clearClaimFileHandle(), clearIehpClaimFile(), clearIehpLoginFile()]);
}
