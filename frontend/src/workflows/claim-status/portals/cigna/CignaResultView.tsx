...
export function CignaResultView({
  errorScreenshots,
  logs,
  onOtpChange,
  onOtpSubmit,
  ...
  const normalizedOtpValue = (otpValue || "").replace(/\D/g, "").slice(0, 6);
  const isOtpComplete = normalizedOtpValue.length === 6;
    ...
    <>
      ...
      {otpRequest ? (
        <div className="mt-4 rounded-[1.4rem] border border-sky-200 bg-[linear-gradient(180deg,rgba(239,246,255,0.95),rgba(224,242,254,0.88))] p-5 shadow-[0_18px_36px_rgba(14,116,144,0.12)]">
          <label className="block text-sm font-semibold uppercase tracking-[0.18em] text-sky-900" htmlFor="cignaOtp">
            {otpRequest.label}
          </label>
