...
import { FileSpreadsheet, KeyRound, Play, SlidersHorizontal, Upload } from "lucide-react";
...
export function AvailityInputForm({
  ...
  inputFileName,
  isProcessing,
  selectionRulesFileName,
  selectedProjectId,
  onCredentialFileChange,
  onInputFileChange,
  onProjectChange,
  onSelectionRulesFileChange,
  onSubmit,
}: {
  canSubmit: boolean;
  ...
  return (
    <form className="space-y-5" onSubmit={onSubmit}>
      ...
      {selectedProjectId === "charm" ? (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-[1rem] border border-sky-100 bg-white/85 px-3 py-2 text-sm">
          <div className="flex min-w-0 items-center gap-2 text-slate-700">
            <span className="flex h-8 w-8 items-center justify-center rounded-[0.8rem] bg-sky-50 text-blue-700">
              <SlidersHorizontal className="h-4 w-4" strokeWidth={2.1} />
            </span>
            <div className="min-w-0">
              <p className="font-semibold text-slate-900">Charm org/provider rules</p>
              <p className="truncate text-xs text-slate-500">
                {selectionRulesFileName || "Optional .xlsx or .csv override"}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {selectionRulesFileName ? (
              <button
                type="button"
                onClick={() => onSelectionRulesFileChange(null)}
                disabled={isProcessing}
                className="rounded-full px-2.5 py-1 text-xs font-semibold text-slate-500 transition hover:bg-slate-100 disabled:opacity-60"
              >
                Clear
              </button>
            ) : null}
            <input
              id="availitySelectionRulesFile"
              type="file"
              accept=".xlsx,.xls,.csv"
              onChange={(event) => {
                onSelectionRulesFileChange(event.target.files?.[0] ?? null);
                event.currentTarget.value = "";
              }}
              className="hidden"
            />
            <label
              htmlFor="availitySelectionRulesFile"
              className="inline-flex cursor-pointer items-center gap-1.5 rounded-full border border-sky-200 bg-sky-50 px-3 py-1.5 text-xs font-semibold text-blue-700 transition hover:border-blue-300 hover:bg-blue-50"
            >
