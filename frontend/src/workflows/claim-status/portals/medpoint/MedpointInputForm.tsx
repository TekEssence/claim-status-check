...
export function MedpointInputForm({
  canSubmit,
  inputFileName,
  isProcessing,
  loginFileName,
    ...
    <form className="space-y-5" onSubmit={onSubmit}>
      <div className="grid gap-5 xl:grid-cols-2">
        <PortalUploadCard
          ...
          description="Upload the Medpoint login workbook with username, password, and any required portal access details."
          fileName={loginFileName}
          icon={KeyRound}
          inputId="medpointLoginExcel"
        ...
        <PortalUploadCard
          ...
          accept=".xlsx,.xls,.csv"
          acceptedFormats=".xlsx, .xls, .csv"
          description="Upload the claim workbook for Medpoint patient and service-line details."
          fileName={inputFileName}
          icon={FileSpreadsheet}
          inputId="medpointInputExcel"
      ...
      <p className="text-center text-sm text-slate-500">Medpoint processing is connected. If captcha appears, complete it in the Medpoint browser window and then continue from this UI.</p>
    </form>
  );
}
