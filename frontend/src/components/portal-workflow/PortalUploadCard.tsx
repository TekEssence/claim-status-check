...
export function PortalUploadCard(props: PortalUploadCardProps) {
  ...
  return (
    <div className="rounded-[1.5rem] border border-sky-100 bg-[linear-gradient(180deg,rgba(255,255,255,0.98)_0%,rgba(244,249,255,0.96)_100%)] p-5 shadow-[0_16px_34px_rgba(148,163,184,0.12)] transition duration-200 hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-[0_20px_40px_rgba(59,130,246,0.12)]">
      <div className="flex items-start justify-between gap-3">
        <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[linear-gradient(180deg,#dbeafe_0%,#bfdbfe_100%)] text-blue-700 shadow-inner">
      ...
      <h3 className="mt-4 text-base font-semibold tracking-[-0.03em] text-slate-950">{props.title}</h3>
      ...
      {props.mode === "file" ? (
        <>
          ...
          <label
            ...
            onDrop={onDrop}
            className={`mt-4 block cursor-pointer rounded-[1.2rem] border border-dashed px-4 py-5 text-center transition ${
              isDragging
          ...
          >
            <CloudUpload className="mx-auto h-8 w-8 text-blue-600" strokeWidth={2.1} />
            <p className="mt-3 text-sm font-medium text-slate-800">
              Drag and drop your file here
            </p>
            <p className="mt-1 text-xs text-slate-500">or use the button below to browse</p>
            <span className="mt-4 inline-flex items-center rounded-full bg-[linear-gradient(90deg,#eff6ff_0%,#dbeafe_100%)] px-4 py-2 text-sm font-semibold text-blue-700 shadow-sm">
              Browse File
            </span>
          </label>
        </>
        ...
        <button
          ...
          className="mt-4 flex w-full items-center justify-center rounded-[1.2rem] border border-dashed border-sky-200 bg-white/80 px-4 py-5 text-center transition hover:border-blue-400 hover:bg-blue-50/45"
        >
          <span>
            <FolderOpen className="mx-auto h-8 w-8 text-blue-600" strokeWidth={2.1} />
            <span className="mt-3 block text-sm font-medium text-slate-800">
              Select the workbook to update in place
            </span>
            <span className="mt-1 block text-xs text-slate-500">Chrome or Edge required for secure file access</span>
            <span className="mt-4 inline-flex items-center rounded-full bg-[linear-gradient(90deg,#eff6ff_0%,#dbeafe_100%)] px-4 py-2 text-sm font-semibold text-blue-700 shadow-sm">
      ...
      {props.fileName ? (
        <div className="mt-4 rounded-[1rem] border border-emerald-100 bg-emerald-50/70 px-4 py-3">
          <div className="flex items-start justify-between gap-3">
      ...
      {props.helperText ? <p className="mt-3 text-xs leading-5 text-slate-500">{props.helperText}</p> : null}
    </div>
  );
}
